...
token = requests.get('https://open.spotify.com/access_token?reason=transport&productType=web_player').text
...
for a in range(0,5000,100):
    ...
    result = json.loads(requests.get('https://api.spotify.com/v1/playlists/'+str(playlist_id)+'/tracks?offset='+str(a)+'&limit=100',headers=headers).text)
    ...
    if len(result['items']) > 0:
        for i in result['items']:
            ...
            data.append([track_name,artists_names,album_name,duration_min,release_date,disc_number,track_number,explicit,added_by,added_at,spotify_id,thumbnail_link])
    else:
        break
...
with f:
    writer = csv.writer(f)
    writer.writerow(['track_name','artists_names','album_name','duration_min','release_date','disc_number','track_number','explicit','added_by','added_at','spotify_id','thumbnail_link'])
